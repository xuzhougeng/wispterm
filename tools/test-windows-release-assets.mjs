...
const installScript = fs.readFileSync("packaging/windows/Install-WispTerm.ps1", "utf8");
...
assert.match(packageScript, /wispterm-ssh-askpass\.exe/);
assert.match(installScript, /wispterm-ssh-askpass\.exe/);

assert.match(build, /\.source_dir = b\.path\("plugins"\)/);
assert.match(build, /\.install_subdir = "plugins"/);
assert.match(build, /wispterm-ssh-askpass/);
