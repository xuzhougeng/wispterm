...
Desktop releases use `packaging/publish_release.py` for their shared release body and publication gate. Keep the generated **Performance baseline / 性能基线** and **Downloads / 下载** sections. Windows must run `packaging/windows/benchmark-release.ps1` and upload both versioned Markdown and JSON reports. The publisher keeps a release as a draft until all desktop packages, the CLI bundle, and both reports are uploaded; do not bypass this by publishing a partial draft manually. Write the changes in `release-notes/vX.Y.Z.md`; the shared script adds the benchmark links and download table automatically. Run `python -m unittest discover -s packaging -p test_publish_release.py` when changing this flow.
