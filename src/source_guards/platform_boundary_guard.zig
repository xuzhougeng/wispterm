//! Source-scan guards for platform-boundary and architecture leaks.
//! These checks stay in the fast suite without compiling the full app.

const std = @import("std");
const scan = @import("scan.zig");

fn fail(comptime message: []const u8) !void {
    std.debug.print("platform_boundary_guard: {s}\n", .{message});
    return error.SourceGuardFailed;
}

test "platform boundary source guards" {
    ...
    if (std.mem.indexOf(u8, source, concrete_font_backend) != null) {
        try fail("test_main.zig must import platform/font_backend.zig, not the concrete DirectWrite backend");
    }

    const app_window_source = @embedFile("../AppWindow.zig");
    if (std.mem.indexOf(u8, app_window_source, "builtin." ++ "os.tag") != null) {
        try fail("AppWindow.zig must keep native handle OS switches behind platform/window_backend.zig");
    }
    if (std.mem.indexOf(u8, app_window_source, "W" ++ "M_") != null) {
        try fail("AppWindow.zig comments and logic must describe platform-neutral event handling, not Win32 messages");
    }
    const app_window_launch_leaks = [_][]const u8{
        "[260]u16",
        "[256]u16",
        "[*:0]const u16",
        "utf8PathToCwdPtr",
        "unixPathToWindows",
        "unixPathToNativeCwd",
        "unixPathToLocalPathUtf8",
        "utf16LeToUtf8",
        "utf8ToUtf16Le(cwd_buf",
        "utf8ToUtf16LeAllocZ",
    };
    if (scan.containsAny(app_window_source, &app_window_launch_leaks)) {
        try fail("AppWindow.zig launch plumbing must use platform_pty_command launch types and allocators");
    }
    ...
    if (std.mem.indexOf(u8, update_install_source, "@import(\"builtin\").os.tag") != null) {
        try fail("update_install.zig must use platform/update_package.zig for OS package selection");
    }
    ...
    if (std.mem.indexOf(u8, platform_threading_source, "Windows") != null) {
        try fail("platform/threading.zig thread policy comments must describe runtime roles, not Windows implementation defaults");
    }
    ...
    {
        try fail("update_install.zig must validate archive entry names through platform/update_package.zig");
    }
    ...
    {
        try fail("update_install.zig must validate release package manifests without WebView2-specific options or errors");
    }
    if (std.mem.indexOf(u8, update_install_source, "windowsPortablePackage") != null) {
        try fail("update_install.zig tests must use platform-neutral update package scenarios");
    }
    ...
    {
        try fail("update_install.zig tests must not assert concrete platform package backends directly");
    }
    ...
    {
        try fail("update_install.zig tests must get executable payload paths through platform/update_package.zig");
    }
    ...
    if (std.mem.indexOf(u8, local_path_source, "pub fn isAbsoluteOrWindows") != null) {
        try fail("platform/local_path.zig public APIs must use platform-neutral path role names");
    }
    ...
    {
        try fail("platform/local_path.zig path helpers must describe native path roles instead of Windows-specific helper names");
    }
    ...
    {
        try fail("App.zig must keep concrete release asset names behind update/release package modules");
    }
    ...
    {
        try fail("App.zig must use platform_pty_command buffer types for launch command and cwd storage");
    }
    ...
    {
        try fail("App/AppWindow launch APIs must use native command line names, not UTF-16-specific names");
    }
    ...
    {
        try fail("ssh_hosts profile schema must only add server auth fields; port forwarding must not extend it");
    }
    ...
    {
        try fail("Existing URL SSH tunnel code must remain local-forwarding only");
    }
    ...
    const shared_pty_leaks = [_][]const u8{
        "ConPTY",
        "ResizePseudoConsole",
        "ReadFile",
        "CancelIoEx",
    ...
    inline for (shared_pty_sources) |pty_source| {
        if (scan.containsAny(pty_source, &shared_pty_leaks)) {
            try fail("shared PTY/termio code must describe platform-neutral PTY behavior");
        }
    }
    ...
    if (std.mem.indexOf(u8, update_check_source, "wispterm-windows-portable") != null) {
        try fail("update_check.zig tests must build concrete release asset names through release_package helpers");
    }
    if (std.mem.indexOf(u8, update_check_source, "ReleasePackage.windowsPortable") != null) {
        try fail("update_check.zig tests must construct concrete platform packages through platform/update_package.zig");
    }
    if (std.mem.indexOf(u8, update_check_source, "windowsPortablePackage") != null) {
        try fail("update_check.zig tests must use platform-neutral update package scenarios");
    }
    ...
    {
        try fail("release_package.zig must keep concrete executable payload names behind platform/update_package backends");
    }
    if (std.mem.indexOf(u8, release_package_source, "wispterm-windows-portable") != null) {
        try fail("release_package.zig must keep concrete asset naming behind platform/update_package backends");
    }
    ...
    {
        try fail("release_package.zig must use platform-neutral package flavors instead of Windows portable fields");
    }
    ...
    {
        try fail("release_package.zig public helpers must describe embedded browser payloads, not WebView2-specific APIs");
    }
    ...
    {
        try fail("update_check.zig tests must use embedded-browser package flavor names");
    }
    ...
    {
        try fail("platform/update_package.zig public APIs must use embedded-browser package flavor names");
    }
    if (std.mem.indexOf(u8, update_package_source, "windows_portable") != null) {
        try fail("platform/update_package.zig facade backend names must describe platforms, not concrete package shapes");
    }
    if (std.mem.indexOf(u8, update_package_source, "windows" ++ "_package") != null) {
        try fail("platform/update_package.zig facade backend aliases must describe package backend roles, not concrete platform package names");
    }
    ...
    {
        try fail("platform/update_package.zig archive validation helpers must use archive-safety names, not Windows implementation names");
    }
    ...
    if (std.mem.indexOf(u8, pty_command_source, "windows_create_process") != null) {
        try fail("platform/pty_command.zig facade backend names must describe platforms, not CreateProcess details");
    }
    if (std.mem.indexOf(u8, pty_command_source, "PseudoConsoleHandle") != null) {
        try fail("platform/pty_command.zig facade must not expose pseudo-console handles");
    }
    ...
    {
        try fail("platform/pty_command.zig public shell APIs must describe local-shell roles, not PowerShell-specific helpers");
    }
    if (std.mem.indexOf(u8, pty_command_source, "appendWindowsQuotedArg") != null) {
        try fail("platform/pty_command.zig command quoting helpers must describe command-line roles, not Windows implementation names");
    }
    if (std.mem.indexOf(u8, pty_command_source, "Windows" ++ "LocalShell") != null) {
        try fail("platform/pty_command.zig local-shell helpers must describe native shell roles, not Windows-local helper names");
    }
    ...
    {
        try fail("platform/pty_command.zig must delegate concrete native shell title mappings to backend implementations");
    }
    ...
    {
        try fail("platform/pty_command.zig must delegate concrete tab, WSL, and SSH command construction to backend implementations");
    }
    ...
    {
        try fail("platform/pty_command.zig must delegate concrete launch-kind command classification to backend implementations");
    }
    if (std.mem.indexOf(u8, pty_command_source, "\"cmd.exe /k ssh.exe") != null or
        std.mem.indexOf(u8, pty_command_source, "\"wsl.exe ~") != null)
    {
        try fail("platform/pty_command.zig facade tests must keep concrete Windows SSH/WSL command-line samples in backend implementations");
    }
    const pty_facade_source = @embedFile("../platform/pty.zig");
    if (std.mem.indexOf(u8, pty_facade_source, "windows_conpty") != null) {
        try fail("platform/pty.zig facade backend names must describe platforms, not ConPTY details");
    }
    ...
    inline for (platform_facade_backend_detail_checks) |check| {
        if (std.mem.indexOf(u8, check[1], check[2]) != null) {
            try fail(check[0] ++ " facade backend names must describe platform roles, not backend implementation details");
        }
    }
    ...
    {
        try fail("Command.zig must start commands through the app-facing PTY API, not pseudo-console handles");
    }
    const surface_source = @embedFile("../Surface.zig");
    if (std.mem.indexOf(u8, surface_source, "pty.pseudo_console") != null) {
        try fail("Surface/AppWindow code must not reach into platform PTY pseudo-console handles");
    }
    ...
    {
        try fail("Surface/Command launch interfaces must use platform_pty_command launch types, not raw Windows UTF-16 pointers");
    }
    ...
    {
        try fail("appwindow/tab.zig launch APIs must use platform_pty_command launch types and allocators");
    }
    ...
    {
        try fail("input.zig comments and logic must describe platform-neutral input events, not Win32 messages");
    }
    ...
    {
        try fail("input.zig must use platform_wsl/platform_pty_command native cwd helpers");
    }
    if (std.mem.indexOf(u8, input_source, "\"{s}\\\\{s}\"") != null) {
        try fail("input.zig must build local filesystem paths through platform/local_path.zig");
    }
    ...
    {
        try fail("shared input APIs must use backend-neutral key_* names instead of Win32 VK aliases");
    }
    ...
    {
        try fail("shared input events must use key_code naming instead of Win32 VK naming");
    }
    ...
    {
        try fail("keybind triggers must use key_code naming instead of Win32 VK naming");
    }
    if (std.mem.indexOf(u8, keybind_source, "VK_{X}") != null) {
        try fail("keybind.zig fallback key labels must not expose Win32 VK terminology");
    }
    ...
    {
        try fail("platform/global_hotkey.zig facade must use key_code naming instead of Win32 VK naming");
    }
    ...
    {
        try fail("input/clipboard.zig must ask platform remote-file helpers to adapt local paths for terminal paste targets");
    }
    ...
    if (std.mem.indexOf(u8, input_source, "g_preview_image_drag.begin(") == null) {
        try fail("input.zig mouse-down must start the image-preview pan drag via PreviewImageDrag.begin");
    }
    if (std.mem.indexOf(u8, input_source, "g_preview_image_drag.move(") == null) {
        try fail("input.zig mouse-move must pan the image preview via PreviewImageDrag.move");
    }
    if (std.mem.indexOf(u8, input_source, "g_preview_image_drag.release(") == null) {
        try fail("input.zig must drop the image-preview pan drag via PreviewImageDrag.release on mouse-up/cancel");
    }
    ...
    {
        try fail("platform/wsl.zig public path APIs must describe host/guest roles, not Windows/Unix implementation details");
    }
    ...
    if (std.mem.indexOf(u8, platform_remote_file_source, "windowsPathToWslPathAlloc") != null) {
        try fail("platform/remote_file.zig must use platform-neutral WSL path adapter names");
    }
    ...
    if (std.mem.indexOf(u8, platform_process_source, "windows_powershell") != null) {
        try fail("platform/process.zig local shell variants must describe shell roles, not Windows-specific platform names");
    }
    ...
    {
        try fail("platform/process.zig must expose local-shell fallback argv as one narrow API instead of public shell variant plumbing");
    }
    ...
    {
        try fail("browser_panel.zig must use platform_webview URL helpers instead of WebView2 UTF-16 details");
    }
    ...
    {
        try fail("platform/open_url.zig must keep native browser-open details in concrete backends");
    }
    ...
    {
        try fail("session_persist.zig must write replace-safe files through platform/atomic_file.zig");
    }
    ...
    {
        try fail("font/manager.zig must load fallback font paths through platform/font_backend.zig");
    }
    ...
    {
        try fail("font/AppWindow shared DPI code must get display baseline DPI from platform/display.zig");
    }
    ...
    {
        try fail("file_explorer.zig must build local filesystem paths through platform/local_path.zig");
    }
    ...
    {
        try fail("config.zig must get shell defaults and generated config text from platform/pty_command.zig");
    }

    if (std.mem.indexOf(u8, app_source, "cfg.shell = \"powershell\"") != null or
        std.mem.indexOf(u8, app_source, "next.shell = \"pwsh\"") != null)
    {
        try fail("App.zig tests must get shell config examples from platform/pty_command.zig");
    }
    ...
    {
        try fail("renderer/overlays.zig must get shell cycling and hint text from platform/pty_command.zig");
    }
    ...
    {
        try fail("renderer/overlays.zig must get optional WSL session launcher layout from platform/pty_command.zig");
    }
    ...
    {
        try fail("renderer/overlays.zig must only launch AI History sources; provider scanning belongs in ai_history modules");
    }
    ...
    {
        try fail("renderer/titlebar.zig must get caption button metrics and style text through platform/window.zig");
    }

    const startup_tabs_source = @embedFile("../startup_tabs.zig");
    if (std.mem.indexOf(u8, startup_tabs_source, "agent_and_powershell") != null) {
        try fail("startup_tabs.zig must name the default pair as agent plus local shell, not PowerShell");
    }
    ...
    {
        try fail("AppWindow.zig must use platform-local shell launcher names instead of PowerShell-specific shared identifiers");
    }
    ...
    {
        try fail("renderer/overlays.zig must use platform-local shell launcher names instead of PowerShell-specific shared identifiers");
    }
    ...
    {
        try fail("command_center_state.zig must get session launcher labels and rows from platform/pty_command.zig");
    }
    ...
    {
        try fail("ai_chat.zig must get local command tool names, labels, default prompts, and tab kind text from platform modules");
    }
    ...
    {
        try fail("ai_chat.zig must ask platform/process.zig for fallback argv without handling shell variants directly");
    }
    ...
    {
        try fail("ai_chat.zig shared tests must use platform-neutral terminal fixtures unless explicitly testing platform prompt variants");
    }
    ...
    if (std.mem.indexOf(u8, agent_detector_source, "detect(\"PowerShell\", \"PS C:\\\\Users> ls\")") != null) {
        try fail("agent_detector.zig generic shell tests must use platform-neutral terminal fixtures");
    }
}
