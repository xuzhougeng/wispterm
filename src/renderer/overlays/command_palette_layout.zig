...
pub fn fieldChrome(layout: Layout, window_height: f32, pad_x: f32, text_h: f32) FieldChrome {
    ...
    const y = rectY(window_height, layout.box_top_px + layout.header_h, layout.filter_h);
    return .{
...
pub fn rowSlot(layout: Layout, window_height: f32, display_row: usize, text_h: f32) RowSlot {
    ...
    return .{
        ...
        .selected_border = .{ .x = layout.box_x + 12, .y = y + 4, .w = layout.box_w - 24, .h = layout.row_h - 8 },
        .selected_fill = .{ .x = layout.box_x + 13, .y = y + 5, .w = layout.box_w - 26, .h = layout.row_h - 10 },
...
pub fn footerTextY(layout: Layout, window_height: f32, text_h: f32) f32 {
    const panel_y = rectY(window_height, layout.box_top_px, layout.box_h);
    return textY(panel_y, layout.footer_h, text_h);
}
...
test "panel chrome converts command palette box to draw rectangles" {
    const l = compute(1200, 800, 40, 20, 8);
...
test "field chrome reserves horizontal padding and centers text" {
    const l = compute(1200, 800, 40, 20, 8);
    const field = fieldChrome(l, 800, 24, 20);
    const expected_y = rectY(800, l.box_top_px + l.header_h, l.filter_h);

    try std.testing.expectEqual(@as(f32, l.box_x + 24), field.field.x);
    try std.testing.expectEqual(expected_y, field.field.y);
    try std.testing.expectEqual(@as(f32, l.box_w - 48), field.field.w);
...
test "row slots map display rows to selected affordance rects" {
    ...
    try std.testing.expectEqual(DrawRect{ .x = l.box_x + 12, .y = row0.row.y + 4, .w = l.box_w - 24, .h = l.row_h - 8 }, row0.selected_border);
    try std.testing.expectEqual(DrawRect{ .x = l.box_x + 13, .y = row0.row.y + 5, .w = l.box_w - 26, .h = l.row_h - 10 }, row0.selected_fill);
}

test "scrollbar geometry tracks visible row window" {
    const l = compute(1200, 800, 40, 20, 100);
