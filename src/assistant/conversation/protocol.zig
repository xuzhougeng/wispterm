...
fn forEachToolSpec(
    ...
    try Filtered.emitTool(ctx, opts, "terminal_list", "List terminal surfaces. scope=owned (default) lists only this Agent's terminals. scope=all exposes metadata only and never grants read, write, focus, close, or file access. Before any terminal write, use terminal_select with an owned surface_id.", "{\"scope\":{\"type\":\"string\",\"description\":\"owned (default) or all (metadata only).\"}}");
    ...
    try Filtered.emitTool(ctx, opts, "terminal_snapshot", "Read a bounded text snapshot from one owned terminal surface or all owned surfaces.", "{\"surface_id\":{\"type\":\"string\",\"description\":\"Optional owned surface id from terminal_list.\"}}");
    try Filtered.emitTool(ctx, opts, "ui_screenshot", "Capture a PNG screenshot of an owned terminal's active tab or focused panel. Pass an owned surface_id, or omit it only when an owned terminal context is selected. The tool returns a local PNG path; when the request came from a chat channel (WeChat or Feishu), call send_attachment with kind=image and that path.", "{\"target\":{\"type\":\"string\",\"description\":\"Optional: focused_panel (default) or active_tab.\"},\"surface_id\":{\"type\":\"string\",\"description\":\"Optional owned terminal surface id from terminal_list.\"}}");
    try Filtered.emitTool(ctx, opts, "terminal_select", platform_pty_command.terminalSelectToolDescription(), "{\"surface_id\":{\"type\":\"string\",\"description\":\"Owned surface id from terminal_list to make the current agent write context.\"}}");
    try Filtered.emitTool(ctx, opts, "terminal_focus", "Focus and activate an owned terminal surface in the visible WispTerm UI. Use this before ui_screenshot when the requested owned panel is not focused. This changes UI focus only; use terminal_select separately before terminal writes.", "{\"surface_id\":{\"type\":\"string\",\"description\":\"Owned surface id from terminal_list to focus in the visible UI.\"}}");
    try Filtered.emitTool(ctx, opts, "terminal_reconnect", "Reconnect an exited owned SSH terminal in place, preserving its tab, split, and ownership. The prior remote command outcome is unknown: do not automatically re-run it; wait and inspect the original surface with terminal_snapshot.", "{\"surface_id\":{\"type\":\"string\",\"description\":\"Owned exited SSH surface id from terminal_list.\"}}");
    ...
    try Filtered.emitTool(ctx, opts, "ssh_session_exec", "Run a POSIX shell command in the selected already-open SSH terminal surface. The surface_id must match the current terminal_select context. Use only when the surface is at a shell prompt and the command returns. Never use heredocs or multiline shell input to create or feed scripts; use write_file for the content, including large or temporary scripts, then run the file separately. For R, Python, Codex, Claude Code, other REPLs, or launching full-screen agent apps, use terminal_repl_exec.", "{\"surface_id\":{\"type\":\"string\",\"description\":\"Selected surface id from terminal_select.\"},\"command\":{\"type\":\"string\"},\"timeout_ms\":{\"type\":\"integer\"}}");
    ...
    try Filtered.emitTool(ctx, opts, "copy_file", "Copy a small binary/artifact file between local workspace, WSL, and SSH without pasting shell commands into a terminal. Not for directories or multi-GB datasets — use transfer_between_contexts for those. Pull mode: omit dest_surface_id and dest_path to copy source_path into local wispterm-files and return local_path, useful before send_attachment. Push mode: set dest_surface_id to an open WSL or SSH terminal to copy a local/Weixin/workspace file to that environment. Relative source paths resolve against source_surface_id cwd or the agent working directory; relative destination paths resolve against dest_surface_id cwd or the agent working directory.", "{\"source_path\":{\"type\":\"string\",\"description\":\"Path to the source file. Relative paths resolve against source_surface_id cwd, or the agent working directory when source_surface_id is omitted.\"},\"source_surface_id\":{\"type\":\"string\",\"description\":\"Optional source terminal id from terminal_list. SSH pulls via scp; WSL uses a host-accessible WSL path; omitted means local.\"},\"dest_surface_id\":{\"type\":\"string\",\"description\":\"Optional destination terminal id from terminal_list. Use an SSH or WSL surface to send a local file there. Omit to copy into local wispterm-files.\"},\"dest_path\":{\"type\":\"string\",\"description\":\"Optional destination file path. Relative paths resolve against destination surface cwd, or the agent working directory for local destinations. If omitted, uses dest_name or the source basename.\"},\"dest_name\":{\"type\":\"string\",\"description\":\"Optional safe destination filename. In pull mode it is placed inside wispterm-files. Must not contain path separators.\"}}");
    try Filtered.emitToolWithRequired(ctx, opts, "transfer_between_contexts", "Transfer one exact file or directory between local disk and an SSH context. Use this for datasets and folders, including multi-GB trees. Never run scp, rsync, or ssh file copies through powershell_exec, shell_exec, or ssh_session_exec. source_context_id and destination_context_id are `local`, an open terminal surface_id from terminal_list, or a saved SSH profile name/host (for example CPU). SSH paths must be exact absolute or ~/ paths; local paths must be exact absolute paths. Globs, filesystem roots, '..', and existing destinations are rejected. Directories copy recursively via scp -r. For a local download, do not guess the destination — ask the user when destination_path is unspecified. copy_file is only for small artifacts such as chat attachments.", "{\"source_context_id\":{\"type\":\"string\",\"description\":\"`local`, an open terminal surface_id, or a saved SSH profile name/host.\"},\"source_path\":{\"type\":\"string\",\"description\":\"SSH: exact absolute or ~/ path. Local: exact existing absolute file or directory path. Globs are rejected.\"},\"destination_context_id\":{\"type\":\"string\",\"description\":\"`local` for a download, or an SSH surface_id/profile name for an upload.\"},\"destination_path\":{\"type\":\"string\",\"description\":\"SSH: exact absolute or ~/ path. Local: exact new absolute file or directory path. Do not guess it — ask the user when unspecified. Globs are rejected.\"}}", &.{ "source_context_id", "source_path", "destination_context_id" });
    try Filtered.emitTool(ctx, opts, "write_file", "Create or overwrite a local, WSL, or SSH text file with exact content. Always use this instead of a shell heredoc for multiline, large, or temporary scripts, even when the script will be deleted after execution. Shows a diff and (unless permission is full) asks for approval. Set surface_id to an open terminal surface to write there; omitted surface_id follows the selected terminal context, or local when none is selected. Relative paths resolve against that surface cwd or the agent working directory.", "{\"path\":{\"type\":\"string\",\"description\":\"File path. Absolute, or relative to the selected surface cwd / working directory.\"},\"content\":{\"type\":\"string\",\"description\":\"Full file content to write.\"},\"surface_id\":{\"type\":\"string\",\"description\":\"Optional terminal surface id. Omit to use the selected terminal context, or local when none is selected.\"}}");
    ...
    try Filtered.emitTool(ctx, opts, "ssh_profile_connect", "Create a new Agent-owned tab connected to a saved WispTerm SSH server profile by its profile name or host.", "{\"profile_name\":{\"type\":\"string\",\"description\":\"Saved SSH profile name or host to open in a new tab.\"}}");
    ...
    try Filtered.emitTool(ctx, opts, "tab_close", "Close an Agent-owned terminal tab by tab_number (the one-based `tab` shown by terminal_list), surface_id, title, or the active terminal tab when no selector is provided. Cannot close another Agent's or the AI chat tab.", "{\"tab_number\":{\"type\":\"integer\",\"description\":\"One-based owned UI tab number — the `tab` value shown by terminal_list.\"},\"tab_index\":{\"type\":\"integer\",\"description\":\"Zero-based tab index (tab_number minus one). Prefer tab_number.\"},\"surface_id\":{\"type\":\"string\",\"description\":\"Owned surface id from terminal_list.\"},\"title\":{\"type\":\"string\",\"description\":\"Owned terminal tab title to close, such as CPU2.\"}}");
    ...
    try Filtered.emitTool(ctx, opts, "wispterm_config", "List or configure WispTerm's main config file. Use action=list with optional query to find editable keys; action=set with key/value to update one known single-value config key; action=remove with key to reset that key to its built-in default. The tool validates booleans, numbers, enums, colors, times, and ports before writing. Repeatable directives such as keybind, palette, and config-file are intentionally not edited by this tool. To include remote Claude Code and Codex histories, set memory-digest-scan-remote=true. Changes are saved to the main config file and applied through normal hot-reload.", "{\"action\":{\"type\":\"string\",\"description\":\"One of: list (default), set, remove.\"},\"query\":{\"type\":\"string\",\"description\":\"Optional filter for action=list, e.g. memory, remote, font.\"},\"key\":{\"type\":\"string\",\"description\":\"Known single-value WispTerm config key, e.g. font-size, theme, ai-agent-permission, remote-enabled, memory-digest-scan-remote.\"},\"value\":{\"type\":\"string\",\"description\":\"Value for action=set. Use the type shown by action=list.\"}}");
    try Filtered.emitTool(ctx, opts, "get_system_time", "Read WispTerm's current local system time on demand. Use this before resolving today, yesterday, or a month/day without a year, especially before memory_search. Do not guess the current date. The result includes a numeric UTC offset.", "{}");
    ...
    if (opts.include_memory) {
        ...
        try Filtered.emitTool(ctx, opts, "memory_search", "Search the local memory digest of past AI agent sessions (Claude Code, Codex, WispTerm Copilot) from this machine and scanned remote hosts. Use when the user asks where or when they did some past work. For an exact calendar day, first call get_system_time whenever the year is omitted or relative, then pass date=YYYY-MM-DD; date searches that whole day and ignores days, and keywords are optional. Otherwise provide 3-6 candidate keywords (topic terms, tool names, Chinese and English variants); any keyword may hit (OR) and results sort by hit count. Returns date, source host, project, summary, session id, and transcript path. If nothing matches and the user names a remote host, fall back to ssh_session_exec on an open SSH surface to that host: grep -rli <keyword> ~/.claude/projects ~/.codex/sessions | head. If digest data is missing entirely, suggest setting memory-digest-scan-remote=true via wispterm_config or running 'Run memory digest now'.", "{\"keywords\":{\"type\":\"array\",\"items\":{\"type\":\"string\"},\"description\":\"Optional 3-6 candidate filter words. Case-insensitive substring match against summary, title, topics, and project; any hit counts (OR).\"},\"date\":{\"type\":\"string\",\"description\":\"Optional exact local digest date, YYYY-MM-DD. Use get_system_time first if the user omitted the year or used a relative day. Bypasses the days window; omit keywords to return the whole day.\"},\"source\":{\"type\":\"string\",\"description\":\"Optional host/source filter, e.g. CPU2. Substring-matched against source id (local, wsl:<distro>, ssh:<profile>).\"},\"days\":{\"type\":\"integer\",\"description\":\"Optional lookback window in days when date is omitted; default 30.\"}}");
    }
...
test "buildRequestJson advertises system time without injecting a timestamp" {
    const a = std.testing.allocator;
    const params = RequestParams{ .model = "m", .system_prompt = "stable system prompt", .protocol = .chat_completions, .thinking_enabled = false, .reasoning_effort = "", .stream = false };
    const json = try buildRequestJson(a, params, &.{}, true);
    defer a.free(json);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"get_system_time\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "stable system prompt") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "Current local system time:") == null);
}
...
test "file-edit tools appear in the tool schema" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, json, "Always use this instead of a shell heredoc") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "even when the script will be deleted after execution") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "Never use heredocs or multiline shell input") != null);
}
...
test "transfer_between_contexts appears in the tool schema" {
    const a = std.testing.allocator;
    var out: std.ArrayListUnmanaged(u8) = .empty;
    defer out.deinit(a);
    try appendToolSchemas(a, &out, .{ .include_memory = false });
    const json = out.items;
    try std.testing.expect(std.mem.indexOf(u8, json, "\"transfer_between_contexts\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"source_context_id\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"destination_context_id\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "Never run scp") != null);
    try std.testing.expect(builtinToolNameReserved("transfer_between_contexts"));
}
