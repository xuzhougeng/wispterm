...
fn forEachToolSpec(
    ...
    try Filtered.emitTool(ctx, opts, "memory_digest_config", "List or configure WispTerm Memory Digest settings. Use action=list to inspect current values; action=set with key/value to update one allowed memory-digest-* key; action=remove with key to reset that key to its built-in default. To include remote Claude Code and Codex histories, set memory-digest-scan-remote=true; this scans Windows WSL plus every saved SSH profile for ~/.claude/projects and ~/.codex/sessions. Changes are saved to the main config file and applied through normal hot-reload.", "{\"action\":{\"type\":\"string\",\"description\":\"One of: list (default), set, remove.\"},\"key\":{\"type\":\"string\",\"description\":\"Allowed keys only: memory-digest-enabled, memory-digest-profile, memory-digest-run-after, memory-digest-scan-remote, memory-digest-backfill-days, memory-digest-max-chars, memory-digest-input-budget-chars.\"},\"value\":{\"type\":\"string\",\"description\":\"Value for action=set. Booleans use true/false, run-after uses HH:MM, numeric keys use unsigned integers, profile is a string.\"}}");
