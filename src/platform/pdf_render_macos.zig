//! macOS PDF rasterizer: system CGPDFDocument + ImageIO via
//! pdf_render_macos_bridge.m (no bundled library).
const std = @import("std");
const pdf_render = @import("pdf_render.zig");

extern fn wisp_pdf_render_page_macos(
    pdf: [*]const u8,
    pdf_len: usize,
    page_index: u32,
    target_width: u32,
...
extern fn wisp_pdf_free_macos(p: ?*anyopaque) void;
...
pub fn renderPage(
    ...
    const rc = wisp_pdf_render_page_macos(pdf.ptr, pdf.len, page_index, target_width_px, &png_ptr, &png_len, &page_count);
    if (rc != 0) {
        return switch (rc) {
            2 => error.InvalidPdf,
