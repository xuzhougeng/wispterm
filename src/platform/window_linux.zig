//! Linux window module: wraps the SDL3 apprt backend for Win32-shaped callers.
//! The Win32 message-bus primitives are no-ops (SDL doesn't use a Win32 message bus).
const sdl = @import("../apprt/sdl.zig");

/// On Linux we store the SDL_Window* as an opaque pointer (same scheme as
/// the macOS backend which stores NSWindow*). window_backend.nativeHandleBits
/// uses @intFromPtr on it to produce a usize key.
...
pub fn dpiForWindow(hwnd: NativeHandle) u32 {
    return sdl.dpiForNativeHandle(@intFromPtr(hwnd));
}
...
pub fn consumeQuitRequest() bool {
    return sdl.consumeQuitRequest();
}

pub fn requestQuit() void {}

pub fn pumpAppEvents(timeout_seconds: f64) void {
    sdl.pumpAppEvents(timeout_seconds);
}
