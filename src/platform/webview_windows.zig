...
extern fn wispterm_webview2_create(
...
extern fn wispterm_webview2_set_bounds(browser: *Browser, left: c_int, top: c_int, right: c_int, bottom: c_int) callconv(.c) void;
...
extern fn wispterm_webview2_loader_available() callconv(.c) c_int;

pub fn loaderAvailable() bool {
    return wispterm_webview2_loader_available() != 0;
}
...
pub fn create(parent: NativeWindowHandle, bounds: anytype, initial_url: Url) ?*Browser {
    return wispterm_webview2_create(parent, bounds.left, bounds.top, bounds.right, bounds.bottom, initial_url.ptr);
}

pub fn setBounds(browser: *Browser, bounds: anytype) void {
    wispterm_webview2_set_bounds(browser, bounds.left, bounds.top, bounds.right, bounds.bottom);
}

pub fn setVisible(browser: *Browser, visible: bool) void {
    wispterm_webview2_set_visible(browser, if (visible) 1 else 0);
}

pub fn focus(browser: *Browser) void {
    wispterm_webview2_focus(browser);
}
...
pub fn isReady(browser: *Browser) bool {
    return wispterm_webview2_is_ready(browser) != 0;
}

pub fn lastError(browser: *Browser) i32 {
    return wispterm_webview2_last_error(browser);
}

pub fn destroy(browser: *Browser) void {
    wispterm_webview2_destroy(browser);
}
