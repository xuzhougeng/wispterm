...
const en = Strings{
    ...
    .memory_settings_help = "Select tools and locations. Changes are saved immediately and apply to the next digest run; existing summaries are kept. SSH servers come from saved SSH profiles.",
...
const zh_CN = Strings{
    ...
    .memory_settings_help = "勾选工具与采集位置，立即保存，下次生成摘要时生效。已有摘要保留。服务器来自已保存的 SSH 配置。",
