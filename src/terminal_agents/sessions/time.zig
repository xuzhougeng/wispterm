...
pub fn localOffsetSeconds() i32 {
    if (comptime builtin.link_libc) {
        ...
        // The fast-test binaries don't link libc, so time.h (and this @cImport) is
        // unavailable there; the comptime gate keeps it out of that graph. Every
        // real app build links libc, so this UTC fallback is test-only.
        return 0;
    }
}
