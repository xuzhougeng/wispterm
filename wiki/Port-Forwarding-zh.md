...
在表单里从 SSH 配置列表选中目标 profile（左/右或空格移动高亮行），端口保持
`7890`（或与你的代理端口一致），保存。一条反向（`-R`）转发会让服务器的 loopback
`127.0.0.1:7890` 连到你本机上的代理。
...
- **Profile** —— 由哪个已保存 SSH profile 承载隧道。表单会列出已保存配置并高亮当前行，左/右或空格移动选择。该 profile 上设置的 `ProxyJump` 会被沿用。
