...
- **命令片段** —— 定义可复用命令，从命令中心一键发送到任意会话 → [[命令片段|Command-Snippets-zh]]
...
- **Agent 终端控制** —— 可选开启的本机 `wisptermctl` API，供脚本和外部 agent 控制终端 → [[Agent 终端控制|Agent-Terminal-Control-zh]]
- **浏览器与 Jupyter 面板** —— 在右侧 WebView 面板打开网址（Windows/macOS）→ [[浏览器与 Jupyter 面板|Browser-Jupyter-Panel-zh]]
