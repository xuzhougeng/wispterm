...
> 在侧边面板浏览本地、WSL 与 SSH 文件，并在不离开终端的情况下预览 Markdown、文本、表格、图片和 PDF。
...
- 按住 `Ctrl`（macOS 上 `Cmd`）并点击终端输出中的 `.md`、`.txt`、`.csv`、`.tsv`、
  源代码或脚本文件（含 R 脚本 `.r` / `.R`）、受支持的图片文件或 `.pdf`，**或**
- 在文件浏览器里双击受支持的文件。
...
- **PDF** —— 由操作系统自带的 PDF 引擎逐页光栅化：Windows 10+ 上是
  `Windows.Data.Pdf`，macOS 上是 CoreGraphics，Linux 上是 `poppler-utils`
  工具（`pdfinfo` / `pdftoppm`，若预览提示缺失，可用如
  `sudo apt install poppler-utils` 安装）。PDF 预览面板聚焦时，`PageUp` /
  `PageDown` 翻页，底部状态栏在 `PDF` 徽标旁以 `N/M` 显示当前页码。不支持
  加密的 PDF。
